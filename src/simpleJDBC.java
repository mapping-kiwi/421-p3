        ...
        try {
            ...
            while( rs.next() ){
                ...
                System.out.printf("%-12d %-12s %-8d %-10s %-15s%n",
                        bookingId, bookingDate, amount, status, payment);
            }
            //step 2: ask user what booking id to select
            System.out.print("Select BookingID: ");
            int selection = scanner.nextInt();
            while(!validBookingIds.contains(selection)){
                System.out.print("Non valid BookingID selected, Please enter a valid BookingId");
            ...
            System.out.printf("%-12s %-12s %-8s %-10s %-15s%n",
                    "BookingID", "Date", "Amount", "BOOKING_STATUS", "Payment");
            System.out.println("-------------------------------------------------------------");
            if (rs.next()) {
